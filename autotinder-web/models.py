...
class Settings(db.Model):
    ...
    # API Configuration
    gemini_api_key = db.Column(db.String(500), nullable=False, default="")
    
    # Package Configuration
    package_name = db.Column(db.String(200), nullable=False, default="com.tinder")
    
    # Decision Criteria
    decision_criteria = db.Column(db.Text, nullable=False, default="Age 20-35, less drinking")
    
    # AI Model Configuration
    model_heavy = db.Column(db.String(100), nullable=False, default="gemini-2.5-flash")
    model_fast = db.Column(db.String(100), nullable=False, default="gemini-2.5-flash")
    
    # Wait Duration Configuration (in seconds)
    short_wait = db.Column(db.Float, nullable=False, default=1.0)
    medium_wait = db.Column(db.Float, nullable=False, default=1.5)
    long_wait = db.Column(db.Float, nullable=False, default=3.0)
    
    # System Prompts
    decision_prompt = db.Column(db.Text, nullable=False, default="""You are an intelligent matchmaker AI agent for a dating app. Your role is to analyze a provided image of a Tinder profile and decide if it matches the user's preferences based on their specified criteria.
...
Respond STRICTLY with a JSON object in this exact format: {"reason": "<1-2 concise sentences explaining why it's liked or disliked, based on the criteria alignment and any genuineness evaluation>", "action": "[LIKE]" or "[DISLIKE]"}—no additional text, no deviations, no explanations outside the reason field.""")
    ...
    ui_resolver_prompt = db.Column(db.Text, nullable=False, default="""You are an intelligent Tinder UI obstruction detector. Your task is to identify content that requires dismissal for normal profile interaction.
...
- "Influencer" profiles primarily focused on promoting external content
...
CORE PRINCIPLE: Only flag content that prevents normal Tinder usage OR advertisement profiles masquerading as real users. Promotional banners that allow continued swiping are acceptable UI states.""")
    ...
    def to_dict(self):
        ...
        return {
            ...
            'ui_resolver_prompt': self.ui_resolver_prompt,
